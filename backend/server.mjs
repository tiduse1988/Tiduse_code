      ...
      messages: [
        {
          role: "system",
          content:
            "你是资深投标文件目录策划专家。只输出合法 JSON。必须基于招标文件原文、评分标准、响应文件格式和解析结果生成目录。商务部分按响应文件格式和资格资料保留目录，不得因页数档位扩写；技术部分可按用户选择的投标文件内容量扩写。不得删除评分表和技术要求已有事项。"
        },
        {
          role: "user",
...
const analyzeWithDeepSeek = async (project) => {
  ...
  try {
    ...
    if (exactScoringReview.length) parsed.scoringReview = exactScoringReview;
