...
const verificationRulesPath = path.join(__dirname, "verification-rules.json");
let verificationRules = [];

const loadVerificationRules = async () => {
  try {
    const raw = await readFile(verificationRulesPath, "utf-8");
    const parsed = JSON.parse(raw);
    verificationRules = Array.isArray(parsed) ? parsed.filter((rule) => rule.status !== "disabled") : [];
  } catch (error) {
    console.warn(`核验规则加载失败: ${error.message}`);
    verificationRules = [];
  }
};

const verificationRulesForPrompt = () => verificationRules.map((rule) => ({
  ruleId: rule.id,
  name: rule.name,
  category: rule.category,
  criteria: rule.criteria,
  specialNotes: rule.notes || "",
  resultType: rule.resultType || "建议确认",
  riskLevel: rule.risk || "中",
  scoreImpact: Boolean(rule.scoreImpact),
  scoreItem: rule.scoreName || "",
...
const readBody = async (req) =>
  new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      body += chunk;
      if (body.length > 650 * 1024 * 1024) {
        reject(new Error("请求体过大，建议分批上传或使用服务器直传"));
        req.destroy();
      }
    });
    req.on("end", () => {
...
const uploadedBidDto = (project, { includePreview = false } = {}) => {
  if (!project?.uploadedBid) return null;
  const extraction = project.uploadedBid.extraction || {};
  const dto = {
    fileName: project.uploadedBid.fileName || "",
    fileSize: project.uploadedBid.fileSize || 0,
    fileSizeText: sizeText(project.uploadedBid.fileSize || 0),
    uploadTime: project.uploadedBid.uploadTime || "",
    status: project.uploadedBid.status || (project.verificationStatus === "verified" ? "verified" : "uploaded"),
    pageCount: extraction.pageCount || 0,
    charCount: extraction.charCount || 0
  };
  if (includePreview) {
    dto.previewPages = (Array.isArray(extraction.pages) ? extraction.pages : [])
      .slice(0, 120)
      .map((page, index) => ({
        page: page.page || index + 1,
        charCount: page.charCount || String(page.text || "").length,
        ocrProvider: page.ocrProvider || page.provider || "",
        text: String(page.text || "").slice(0, 6000)
      }))
      .filter((page) => page.text.trim());
    if (!dto.previewPages.length && extraction.fullText) {
      dto.previewPages = String(extraction.fullText)
        .match(/[\s\S]{1,5000}/g)
        ?.slice(0, 120)
        .map((text, index) => ({ page: index + 1, text, charCount: text.length, ocrProvider: extraction.ocrProvider || "" })) || [];
    }
  }
  return dto;
};
...
const projectDto = (db, project) => {
  ...
  if (normalized.uploadedBid) {
    normalized.uploadedBid = uploadedBidDto(project, { includePreview: true });
  }
...
const projectListDto = (db, project) => {
  ...
  return {
    ...
    uploadedBid: uploadedBidDto(project),
    owner: publicUser(db.users.find((user) => user.id === project.ownerId) || {}),
...
const saveMultipartFile = async (req, prefix) =>
  new Promise((resolve, reject) => {
    const contentType = String(req.headers["content-type"] || "");
    if (!contentType.includes("multipart/form-data")) {
      reject(new Error("上传格式错误"));
      return;
    }
    ...
    busboy.on("file", (fieldName, file, info) => {
      if (fieldName !== "file" || savedFile) {
        file.resume();
        return;
      }

      const originalFileName = decodeMultipartFileName(info?.filename || "投标文件.docx");
      const fileName = safeFileName(originalFileName || "投标文件.docx");
      const ext = path.extname(fileName).toLowerCase();
      if (![".pdf", ".doc", ".docx"].includes(ext)) {
        file.resume();
        fail(new Error("仅支持 PDF、DOC、DOCX 格式的投标文件"));
        return;
      }

      const uploadPath = path.join(uploadsDir, `${prefix}-${fileName}`);
      const output = createWriteStream(uploadPath);
      let fileSize = 0;
      file.on("data", (chunk) => {
      ...
      file.pipe(output);
      fileWrite = new Promise((finish, error) => {
    ...
    busboy.on("finish", async () => {
      ...
      try {
        await fileWrite;
        if (!savedFile) throw new Error("请选择要上传的投标文件");
        settled = true;
        resolve(savedFile);
      } catch (error) {
        fail(error);
...
const extractUploadedBidDocument = async (filePath, fileName = "") => {
  if (!filePath) {
    return {
      fileName,
      ...
      charCount: 0,
      fullText: "",
      warnings: ["未收到可提取的投标文件。"]
    };
  }
  return runExtractor(filePath);
};
...
const stripParsedTenderBlocks = (content) => {
  ...
  blocks.forEach((pattern) => {
    output = output.replace(pattern, "");
  });
  output = output
    .replace(
      "上下文说明：以下内容来自服务端对原始招标文件的完整提取和解析报告结构化结果。原文过长时，系统会优先保留评分办法、响应文件格式、采购需求、技术参数、资格资料、废标条款及相邻页，不能把未展示的非关键页理解为不存在。",
      "上下文说明：以下内容是按页保留的招标文件原始内容包，是本次请求的唯一事实依据；当前未展示的页面不能理解为不存在。"
    )
    .replace("【招标文件关键章节原文包】", "【招标文件原始内容包（按页保留，必须优先依据）】")
    .replace("【招标文件关键章节原文包，仅作为背景和约束，不得复制成正文】", "【招标文件原始内容包（按页保留，必须优先依据；不得复制成正文）】")
    .replace("【招标文件正文】", "【招标文件原始内容包（按页保留，必须优先依据）】");
  return output.replace(/\n{3,}/g, "\n\n").trim();
};

const requestDeepSeek = async ({ apiKey, messages, maxTokens = 6000, temperature = 0.2, signal }) => {
  const sourceMessages = (Array.isArray(messages) ? messages : []).map((message) => {
...
const technicalHintRules = [
  {
    pattern: /证书|证明|学历|毕业|学位|资质|资格|材料/,
    hint: "材料要求、有效性核验、人员对应、原件核验、归档说明"
  },
  {
    pattern: /总体|架构|目标/,
    hint: "建设目标、总体架构、服务范围、关键能力、成果指标"
  },
  {
    pattern: /方式|模式/,
    hint: "服务模式、人员安排、工具支撑、沟通机制、考核方式"
  },
  {
    pattern: /流程|闭环/,
    hint: "流程节点、受理机制、处理步骤、升级机制、闭环确认"
  },
  {
    pattern: /交付物|清单|文档|资料/,
    hint: "交付清单、编制标准、审核流程、提交周期、归档要求"
  },
  {
    pattern: /持续改进|改进|优化/,
    hint: "改进目标、数据分析、优化措施、跟踪验证、经验沉淀"
  },
  {
    pattern: /服务内容|工作内容|任务/,
    hint: "服务范围、工作任务、执行方法、质量要求、输出成果"
  },
  {
    pattern: /应急|预案|突发|灾备|恢复/,
    hint: "应急组织、响应分级、处置流程、恢复目标、演练复盘"
  },
  {
    pattern: /风险|隐患|问题|缺陷/,
    hint: "风险识别、风险评估、预防措施、处置责任、跟踪闭环"
  },
  {
    pattern: /安全|保密|等保|数据保护|权限|漏洞|审计/,
    hint: "安全策略、权限控制、数据保护、审计留痕、保密承诺"
  },
  {
    pattern: /质量|保证|控制|校核/,
    hint: "质量目标、检查机制、复核流程、问题整改、记录归档"
  },
  {
    pattern: /运维|维护|巡检|运行|值守|监测/,
    hint: "巡检计划、故障响应、服务台受理、工单闭环、运行报告"
  },
  {
    pattern: /售后|质保|持续支持/,
    hint: "服务渠道、响应时限、备件保障、回访机制、持续改进"
  },
  {
    pattern: /设备|仪器|参数|性能指标|雨量计|水位计|ADCP|RTU|终端|全站仪|水准仪|RTK|球机|探照灯|雷达|船/,
    hint: "参数逐项响应、证明材料、偏离说明、安装调试、验收保障"
  },
  {
    pattern: /评分|得分|评审/,
    hint: "评分点拆解、响应章节索引、证明材料、得分保障"
  },
  {
    pattern: /实施|进度|计划|措施/,
    hint: "实施步骤、进度安排、责任分工、过程控制、成果提交"
  },
  {
    pattern: /人员|团队|组织|职责|负责人/,
    hint: "组织架构、岗位职责、人员资质、协作机制、替补安排"
  },
  {
    pattern: /验收|交付|成果/,
    hint: "交付清单、验收依据、资料移交、整改闭环、归档要求"
  },
  {
    pattern: /服务|保障|支撑/,
    hint: "服务目标、服务流程、资源配置、质量监督、成果交付"
  }
];

const technicalHintFor = (value) => {
  const title = compactOutlineLabel(value);
  const matched = technicalHintRules.find((rule) => rule.pattern.test(title));
  ...
  try {
    ...
    return {
      ...
      notes: [
        ...
        `已按“${rangeMeta.label}”档位调用 DeepSeek 生成目录，技术目录 ${normalized.technicalPart.length} 项；事实依据为招标文件原始内容包。`
      ]
    };
  } finally {
...
const TIME_REFERENCE_ONLY_PATTERN = /^(?:同上|同前|见上(?:文|述)?|与(?:开标|投标截止|响应截止|磋商截止|报价截止)时间(?:一致|相同)|同(?:开标|投标截止|响应截止|磋商截止|报价截止)时间)$/;
...
const locateTimeSourcePage = (time, node, extraction) => {
  const pages = Array.isArray(extraction?.pages) ? extraction.pages.filter((page) => String(page?.text || "").trim()) : [];
  const normalizedTime = normalizeTimeText(time);
  const exactPage = pages.find((page) => {
    const text = String(page.text || "");
    return normalizedTime && normalizeTimeText(text).includes(normalizedTime);
  });
  if (exactPage?.page) return `第${exactPage.page}页`;
  const fullText = String(extraction?.fullText || "");
  return normalizedTime && normalizeTimeText(fullText).includes(normalizedTime) ? "原文已定位，页码未提取" : "未定位";
};

const validateKeyDatesAgainstSource = (keyDates, extraction) => {
  if (!Array.isArray(keyDates)) return [];
  return keyDates.map((row) => {
    const current = row && typeof row === "object" ? { ...row } : { node: "时间节点", time: String(row || "") };
    const node = String(current.node || current.item || "时间节点").trim();
    const time = String(current.time || current.content || current.info || "").trim();
    ...
    if (shouldInvalidate) {
      const reason = invalidReference
        ? "招标文件仅使用“同上/与相关时间一致”等关联表述，未提供可核验的具体日期和时间"
        : !hasExactTime
          ? "未提取到完整的具体日期和时间"
          : "提取的具体时间未在招标文件原文中定位";
      return {
...
const annotateRowsWithSource = (rows, extraction, fields = []) => {
  ...
  return rows.map((row) => {
    ...
    const sourcePage = sourcePageForSnippet(content, extraction);
    return {
      ...current,
      sourcePage,
      sourceStatus: sourcePage === "未定位" ? "未定位，需人工核实" : "已在原文定位"
    };
  });
};
...
const deriveRejectionClausesFromExtraction = (extraction = {}) => {
  ...
  for (const page of pages) {
    const text = cleanRequirementSnippet(page?.text || "");
    if (!text || !/废标|无效投标|否决投标|投标无效|不予受理|无效响应/.test(text)) continue;
    const clauses = text.match(/[^。；\n]*(?:废标|无效投标|否决投标|投标无效|不予受理|无效响应)[^。；\n。]*/g) || [];
    for (const clause of clauses.slice(0, 8)) {
      const value = clause.trim();
      if (value.length < 12 || rows.some((row) => row.clause.slice(0, 60) === value.slice(0, 60))) continue;
      rows.push({
        clause: value,
        risk: "招标文件原文列明的废标/无效情形",
        action: "按原文逐项核对投标文件",
        sourcePage: page.page ? `第${page.page}页` : "未定位"
      });
    }
  }
...
const auditScoringCompleteness = (rows, extraction) => {
  ...
  const total = list.reduce((sum, row) => sum + Number(scoreMax(row?.score || 0) || 0), 0);
  const sourceText = String(extraction?.fullText || "");
  const expectsHundred = /(?:满分|总分|合计)\s*[:：]?\s*100\s*分|总分为\s*100/.test(sourceText);
  const status = expectsHundred ? (total === 100 ? "完整" : "可能不完整") : "未明确";
  return {
    expectedTotal: expectsHundred ? 100 : null,
    extractedTotal: total,
    status,
    warning: status === "可能不完整"
      ? `评分标准原文明确总分为100分，但当前提取最高分合计为${total}分，可能存在跨页、合并单元格或漏项，请人工核对。`
      : ""
  };
};
...
const analyzeWithDeepSeek = async (project) => {
  ...
  try {
    ...
    parsed.scoringCompleteness = auditScoringCompleteness(
      exactScoringRowsForAudit.length ? exactScoringRowsForAudit : parsed.scoringReview,
      extraction
    );
    if (parsed.scoringCompleteness.warning) {
      parsed.extractionQuality = parsed.extractionQuality || {};
      parsed.extractionQuality.warnings = [
        ...(Array.isArray(parsed.extractionQuality.warnings) ? parsed.extractionQuality.warnings : []),
        parsed.scoringCompleteness.warning
      ];
    ...
    for (const row of [...deriveRejectionClausesFromExtraction(extraction), ...(Array.isArray(parsed.rejectionClauses) ? parsed.rejectionClauses : [])]) {
      ...
      rejectionRows.push({
        ...row,
        clause,
        risk: row.risk || "招标文件原文列明的废标/无效情形",
        action: row.action || row.responsePoint || "按原文逐项核对投标文件",
        sourcePage: row.sourcePage || row.source || sourcePageForSnippet(clause, extraction)
      });
    }
    parsed.rejectionClauses = annotateRowsWithSource(rejectionRows, extraction, ["clause", "risk"]);
    parsed.qualificationCompliance = parsed.qualificationCompliance || {};
    parsed.qualificationCompliance.qualificationReview = annotateRowsWithSource(
      parsed.qualificationCompliance.qualificationReview,
      extraction,
      ["item", "requirement", "evidence"]
    );
    parsed.qualificationCompliance.certificateChecklist = annotateRowsWithSource(
      parsed.qualificationCompliance.certificateChecklist,
      extraction,
      ["name", "issuer", "validity", "source"]
    );
    parsed.businessReview = annotateRowsWithSource(parsed.businessReview, extraction, ["item", "requirement"]);
    parsed.technicalReview = annotateRowsWithSource(parsed.technicalReview, extraction, ["item", "requirement"]);
    parsed.submissionFormat = annotateRowsWithSource(parsed.submissionFormat, extraction, ["item", "requirement"]);
    parsed.materialsChecklist = annotateRowsWithSource(parsed.materialsChecklist, extraction, ["material", "source", "note"]);
    if (selectedLot) {
...
// Preserve page boundaries so DeepSeek can cite and reason about the original tender text.
// The structured report is intentionally not used as the source of truth for downstream tasks.
const buildRawDocumentText = (extraction = {}, fullText = "", maxChars = 180000) => {
  const pages = Array.isArray(extraction.pages) ? extraction.pages : [];
  const pageChunks = pages
    .map((page, index) => {
      const text = pageText(page).trim();
      if (!text) return "";
      const pageNo = page.page || index + 1;
      return `【招标文件原文｜第${pageNo}页】\n${text}`;
    })
    .filter(Boolean);
  ...
  for (const chunk of pageChunks) {
    if (total + chunk.length > maxChars) {
      ...
      if (remaining > 120) chunks.push(`${chunk.slice(0, remaining)}\n【原始内容包达到本次请求上限，后续页在其他分段请求中继续处理】`);
      break;
    }
    chunks.push(chunk);
    total += chunk.length;
  ...
  return {
    text: chunks.join("\n\n---\n\n"),
    mode: chunks.length === pageChunks.length ? "招标文件原始全文（按页保留）" : "招标文件原始内容分段（按页保留，后续页按章节请求）",
    selectedPages: chunks.map((_, index) => `第${pages[index]?.page || index + 1}页`)
  };
};
...
const buildBidGenerationContext = (project, raw = {}) => {
  ...
  const rawDocument = buildRawDocumentText(extraction, fullText);
  const tenderTables = selectTenderTables(extraction.tables || []);
  const structuredReportText = buildStructuredReportText(raw);

  return {
    textMode: rawDocument.mode,
    tenderText: rawDocument.text,
    rawDocumentText: rawDocument.text,
    rawDocumentMode: rawDocument.mode,
    tenderTables,
    structuredReportText,
    selectedPages: rawDocument.selectedPages,
    extractionQuality: {
      pageCount: extraction.pageCount || 0,
...
const generateVerificationWithDeepSeek = async (project, result) => {
  ...
  const uploadedBidText = String(project.uploadedBid?.extraction?.fullText || "");
  const generatedBidText = project.bidDocument?.technicalChapters?.length ? bidDocumentPlainText(project.bidDocument) : "";
  const bidText = uploadedBidText || generatedBidText;
  if (!bidText.trim()) throw new Error("请先上传标书或生成投标文件，再进行 DeepSeek 核验");

  const raw = result.raw || {};
  const tenderContext = buildBidGenerationContext(project, raw);
  const bidSourceName = project.uploadedBid?.fileName || "系统生成投标文件";
  const bidSourceSummary = project.uploadedBid
    ? `用户上传标书：${project.uploadedBid.fileName}，页数：${project.uploadedBid.extraction?.pageCount || "未明确"}，字符数：${project.uploadedBid.extraction?.charCount || uploadedBidText.length}`
    : `系统生成标书，技术章节数：${project.bidDocument?.technicalChapters?.length || 0}`;
  const configuredRules = verificationRulesForPrompt();
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 120000);
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "你是专业标书核验专家。只输出合法 JSON。你必须对照招标文件原始内容、投标文件和后台配置的核验规则逐项核验，不得把结构化解析报告当作事实来源，不得使用示例数据或自行臆造规则。后台配置规则是本次核验的强制检查清单，必须逐条执行并在结果中体现。重点检查硬性条款、评分项响应、技术要求覆盖、商务/附件材料缺失、格式和风险。不能因为投标文件是AI生成就默认通过；缺少真实资质、证书、业绩、报价等材料必须标为待补/高风险。"
        },
        {
          role: "system",
          content: `后台核验表述规则（必须逐条执行）：${JSON.stringify(configuredRules)}。输出问题时尽量带对应 ruleId、ruleName 和 category；没有发现问题的规则也必须计入 checkedItems 和 passedCount。`
        },
        {
          role: "system",
          content:
            "处理结果必须按以下等级输出，不得把所有问题都写成建议确认：废标风险=命中招标文件明确废标条款、项目编号/项目名称/招投标主体错误、资格准入或保证金关键要素缺失、报价超过限价、暗标或签章硬性违规等可能直接导致投标无效；必须修复=提交前必须补齐或改正的材料、响应、页码对应或证明要素，当前不足以直接认定废标但不能带缺陷提交；影响评分=不影响形式有效性但会导致评分项扣分、技术方案得分下降或证据不足；建议确认=需要人工结合原件、上下文或招标人解释确认的事项；提示=排版、格式、编号规范等不直接影响有效性或评分的提醒。项目编号/项目名称一旦与招标文件不一致，优先判为废标风险。"
        },
        {
          role: "user",
          content: `请对以下投标文件进行核验。\n\n项目名称：${project.name}\n招标文件名：${project.fileName}\n待核验标书来源：${bidSourceSummary}\n待核验标书文件名：${bidSourceName}\n\n【招标文件解析结果】\n项目基础信息：${JSON.stringify(raw.basicReview || {})}\n资格审查：${JSON.stringify(raw.qualificationCompliance || {})}\n商务要求：${JSON.stringify(raw.businessReview || [])}\n技术要求：${JSON.stringify(raw.technicalReview || [])}\n评分标准：${JSON.stringify(raw.scoringReview || [])}\n废标条款：${JSON.stringify(raw.rejectionClauses || [])}\n文件格式：${JSON.stringify(raw.submissionFormat || [])}\n资料清单：${JSON.stringify(raw.materialsChecklist || [])}\nDeepSeek生成目录：${JSON.stringify(project.outlineDocument || raw.bidOutline || {})}\n\n【招标文件表格】\n${JSON.stringify(tenderContext.tenderTables).slice(0, 50000)}\n\n【招标文件正文】\n${tenderContext.tenderText.slice(0, 90000)}\n\n【待核验投标文件】\n${bidText.slice(0, 120000)}\n\n输出 JSON：\n{\n  "overallStatus": "通过/需修改/高风险",\n  "score": 0到100的整数,\n  "summary": "核验摘要，说明主要风险和可提交程度",\n  "checkedItems": 数字,\n  "passedCount": 数字,\n  "mustFixCount": 数字,\n  "warningCount": 数字,\n  "scoreEstimate": {\n    "totalFullScore": 数字,\n    "totalEstimatedScore": 数字,\n    "businessFullScore": 数字,\n    "businessEstimatedScore": 数字,\n    "technicalFullScore": 数字,\n    "technicalEstimatedScore": 数字,\n    "items": [{\"category\":\"商务/技术/报价/其他\",\"scoringItem\":\"评分项\",\"fullScore\":数字,\"estimatedScore\":数字,\"basis\":\"按评分表和投标文件推算依据\",\"risk\":\"扣分风险或待确认事项\",\"page\":\"投标文件对应页码或未定位\"}]\n  },\n  "dimensions": [{\"name\":\"资格材料完整性/实质性条款响应/技术评分响应/格式签章/报价与商务/附件材料\",\"status\":\"通过/需修改/高风险/待补\",\"detail\":\"核验说明\"}],\n  "issues": [{\"severity\":\"必须修复/建议确认/提示\",\"category\":\"分类\",\"item\":\"核验项\",\"tenderRequirement\":\"招标要求原文或摘要\",\"bidEvidence\":\"投标文件中对应内容或未找到\",\"problem\":\"发现的问题\",\"suggestion\":\"具体修订建议\",\"location\":\"建议定位章节\",\"page\":\"错误或风险所在投标文件页码，无法定位则写未定位\"}],\n  "missingMaterials": [{\"material\":\"缺失或待补材料\",\"reason\":\"原因\",\"suggestion\":\"补充建议\"}],\n  "coverage": [{\"requirement\":\"招标技术/评分要求\",\"status\":\"已响应/部分响应/未响应\",\"bidSection\":\"对应投标章节\",\"suggestion\":\"补强建议\"}]\n}\n要求：\n1. 只基于上述招标文件和投标文件核验，不要使用静态示例。\n2. 如果用户上传了标书，必须以用户上传标书为准，不要改用系统生成标书。\n3. 必须基于评分标准逐项推算得分，scoreEstimate 要显示满分多少、预计得分多少；无法确定的评分项也要列出并说明待人工确认。\n4. 技术正文可以判断覆盖程度；商务资质、证书、财务、纳税、社保、报价、签章等如果投标文件只保留目录，必须提示“待投标人补充/人工核验”。\n5. 问题清单要可执行，每个问题尽量给出投标文件错误页码；无法定位页码时 page 写“未定位”。\n6. 不要 Markdown，不要解释。`
        }
      ]
    });
    const content = data.choices?.[0]?.message?.content || "";
    ...
    return {
      ...
      bidSource: project.uploadedBid ? "uploaded" : "generated",
      bidFileName: project.uploadedBid?.fileName || "系统生成投标文件.docx",
      ...
      checkedItems: Number(parsed.checkedItems || configuredRules.length || 0),
      ...
      scoreEstimate: parsed.scoreEstimate && typeof parsed.scoreEstimate === "object" ? parsed.scoreEstimate : null,
      issues: Array.isArray(parsed.issues)
        ? parsed.issues.map((issue) => ({ ...issue, page: issue.page || issue.pageNo || issue.locationPage || "未定位" }))
        : [],
...
const verificationReportHtml = (project, verification = {}) => {
  ...
  const scoreEstimate = verification.scoreEstimate || {};
  const scoreItems = Array.isArray(scoreEstimate.items) ? scoreEstimate.items : [];
  return `
    ...
    <h3>得分推算</h3>
    <p><strong>总分：</strong>${escHtml(scoreEstimate.totalEstimatedScore ?? "-")} / ${escHtml(scoreEstimate.totalFullScore ?? "-")}；<strong>商务分：</strong>${escHtml(scoreEstimate.businessEstimatedScore ?? "-")} / ${escHtml(scoreEstimate.businessFullScore ?? "-")}；<strong>技术分：</strong>${escHtml(scoreEstimate.technicalEstimatedScore ?? "-")} / ${escHtml(scoreEstimate.technicalFullScore ?? "-")}</p>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr><th style="border:1px solid #ddd;padding:6px;">类别</th><th style="border:1px solid #ddd;padding:6px;">评分项</th><th style="border:1px solid #ddd;padding:6px;">满分</th><th style="border:1px solid #ddd;padding:6px;">预计得分</th><th style="border:1px solid #ddd;padding:6px;">依据/风险</th></tr></thead>
      <tbody>${scoreItems.map((item) => `<tr><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.category)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.scoringItem || item.item)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.fullScore)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.estimatedScore)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.basis || item.risk)}</td></tr>`).join("") || '<tr><td colspan="5" style="border:1px solid #ddd;padding:6px;">暂无得分推算</td></tr>'}</tbody>
    </table>
    <h3>一、核验维度</h3>
    <table style="width:100%;border-collapse:collapse;">
    ...
    <h3>二、问题清单</h3>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr><th style="border:1px solid #ddd;padding:6px;">等级</th><th style="border:1px solid #ddd;padding:6px;">分类</th><th style="border:1px solid #ddd;padding:6px;">错误页码</th><th style="border:1px solid #ddd;padding:6px;">问题</th><th style="border:1px solid #ddd;padding:6px;">修订建议</th></tr></thead>
      <tbody>${issues
        .map((item) => `<tr><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.severity)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.category)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.page || item.location || "未定位")}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.problem || item.item)}</td><td style="border:1px solid #ddd;padding:6px;">${escHtml(item.suggestion)}</td></tr>`)
        .join("") || '<tr><td colspan="5" style="border:1px solid #ddd;padding:6px;">未发现明确问题</td></tr>'}</tbody>
    </table>
    <h3>三、缺失/待补材料</h3>
...
const renderAnalysisReport = (project, result) => {
  ...
  return `
    ...
    ${tableHtml("2.关键时间节点表", ["时间节点", "具体时间", "重要提醒", "原文页码"], raw.basicReview?.keyDates)}
    ...
    ${tableHtml("4.评分标准表", ["评分项", "分值", "评分说明", "响应策略", "原文页码"], raw.scoringReview)}
    <p><strong>评分表完整性校验：</strong>${escHtml(raw.scoringCompleteness?.status || "未明确")}；提取最高分合计 ${escHtml(raw.scoringCompleteness?.extractedTotal ?? "未明确")} 分${raw.scoringCompleteness?.expectedTotal ? `，招标文件预期总分 ${escHtml(raw.scoringCompleteness.expectedTotal)} 分` : ""}。</p>
    ${tableHtml("5.废标风险表", ["条款", "风险", "处理建议", "原文页码"], raw.rejectionClauses)}
    <h2>D.文件与大纲</h2>
...
const renderAnalysisPdf = async (project, result) => {
  ...
  return pdfBuffer((doc) => {
    ...
    table(doc, "重要时间节点", ["时间节点", "具体时间", "重要提醒", "原文页码"], raw.basicReview?.keyDates, ["node", "time", "reminder", "sourcePage"]);
    ...
    table(doc, "评分标准", ["评分项", "分值", "评分说明", "响应策略", "原文页码"], raw.scoringReview, ["category", "score", "criteria", "responseStrategy", "sourcePage"]);
...
const handleApi = async (req, res, url) => {
  ...
  if (projectAction) {
    ...
    if (req.method === "POST" && action === "upload-bid") {
      if (project.status !== "completed") {
        sendJson(res, 400, { error: "招标文件尚未解析完成，不能上传标书核验" });
        return;
      }
      const uploaded = await saveMultipartFile(req, `${project.id}-bid`);
      const extraction = await extractUploadedBidDocument(uploaded.filePath, uploaded.fileName);
      project.uploadedBid = {
      ...
      project.verificationStatus = "not_started";
      project.verificationMessage = "已上传标书，待开始核验";
      project.verificationDocument = null;
      await writeDb(session.db);
      sendJson(res, 200, { project: projectDto(session.db, project) });
      return;
    ...
    if (req.method === "POST" && action === "verify") {
      ...
      if (!project.uploadedBid?.extraction?.fullText && !project.bidDocument?.technicalChapters?.length) {
        sendJson(res, 400, { error: "请先上传标书或生成投标文件，再进行核验" });
        return;
      }
      const result = projectResult(session.db, project.id);
      ...
      try {
        ...
        if (latestProject.uploadedBid) latestProject.uploadedBid.status = "verified";
        await writeDb(latest);
        sendJson(res, 200, { project: projectDto(latest, latestProject) });
      } catch (error) {
        const latest = await readDb();
